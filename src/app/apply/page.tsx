...
import q1positive from '@/assets/old/question_images/q1/agree.svg'
import q1negative from '@/assets/old/question_images/q1/disagree.svg'
import q2positive from '@/assets/old/question_images/q2/agree.svg'
import q2negative from '@/assets/old/question_images/q2/disagree.svg'
import q3positive from '@/assets/old/question_images/q3/agree.svg'
import q3negative from '@/assets/old/question_images/q3/disagree.svg'
import q4positive from '@/assets/old/question_images/q4/agree.svg'
import q4negative from '@/assets/old/question_images/q4/disagree.svg'
    ...
    return (
        <div className="h-screen bg-slate-200">
            <div className="w-full bg-yellow-600 p-5">
                <p className="text-xl">문제의 상황 설명과 선택지의 근거를 바탕으로 선택해주세요. 총 {Object.keys(answer).length}문항입니다.</p>
            </div>
            <StateDisplayer current={count} answers={answer} setCurrent={handleSetCurrent}></StateDisplayer>
            {questions.length > 0 && <QuestionDisplayer question={questions[count - 1]} answer={answer[count]} answerUpdate={answerUpdate}></QuestionDisplayer>}
        </div>
    )
}
function QuestionDisplayer({ question, answer, answerUpdate }: { question: Question, answer: Answer, answerUpdate: (newAnswer: Answer) => void }) {
    ...
    const { question_num, question_text, positive_text, negative_text } = question;
    useEffect(() => {
    ...
    return <>
        ...
        <div className="flex gap-20 justify-center">
            <div className={"w-[400px] bg-slate-100 rounded-xl drop-shadow-xl" + (clicked === 1 ? " border-4 border-red-400" : "")}
                onClick={() => clickHandler(1)}>
                <Image src={`./assets/old/question_images/q${question_num}/agree.svg`} width={400} height={400} alt="이거 뭐냐"></Image>
                <p className="text-center bg-green-600 py-2 font-bold  text-lg">찬성</p>
                <p className=" text-black p-3">{positive_text}</p>
            </div>
            <div className={"w-[400px] bg-slate-100 rounded-2xl drop-shadow-xl" + (clicked === 2 ? " border-4 border-red-400" : "")}
                onClick={() => clickHandler(2)}>
                <Image src={`./assets/old/question_images/q${question_num}/disagree.svg`} width={400} height={400} alt="이거 뭐냐"></Image>
                <p className="text-center bg-blue-500 py-2 font-bold  text-lg">반대</p>
        ...
        <div className="flex flex-col justify-center mt-2  gap-4">
            <div className="flex justify-center">
                <div>
                    <label htmlFor="option" className="label">의견을 남겨주세요.(생략가능)</label>
                    <input id="option" ref={optionInput} className="w-[800px] input input-bordered border-gray-400 drop-shadow-xl bg-gray-100 text-black " placeholder="위 문항에 대한 시장님의 의견을 들려주세요!" onChange={change_hanler} value={optionText}></input>
                </div>
            </div>
            <button className="btn btn-warning mx-[30px]" disabled={clicked === -1} onClick={submitHandler}>제출</button>
        </div>
    </>
}
function StateDisplayer({ answers, current, setCurrent }: { answers: { [key: number]: Answer }, current: number, setCurrent: (num: number) => void }) {
    ...
    return <div className="flex w-full justify-center gap-5 my-5 ">
        {data.map((k, i) => {
            ...
            if (current === idx) {
                return <div key={idx} onClick={() => setCurrent(idx)} className={` w-5 h-5 border-4 bg-blue-600 border-yellow-300   rounded-full`} >
                </div>
            }
            else {
                if (answers[k].answer === -1) {
                    return <div key={idx} onClick={() => setCurrent(idx)} className={` w-5 h-5 border-4 bg-gray-700 border-gray-700 rounded-full`} >
                    </div>
                } else {
                    return <div key={idx} onClick={() => setCurrent(idx)} className={` w-5 h-5 border-4 bg-green-700 border-green-700 rounded-full`} >
                    </div>
                }
            }
        })
