...
function OpinionDisplayer(
    ...
    const [current_paging, setCurrentPaging] = useState(-1);
    ...
    useEffect(() => {
        ...
        setCurrentPaging(0);
        setOpinion([]);
        setQuery(query_filter);

    }, [current_page, age, grade, gender, idx]);
    ...
    function get_opinion(start_idx: number) {
        supaclient
            ...
            .match(query)
            .neq('answer_text', '')
            .range(start_idx * page_size, (start_idx + 1) * page_size - 1)
            .then(({ data, error }) => {
                if (!data) return;
                const temp: string[] = [];
                console.log("돌았다");
            ...
            <div className="grid grid-cols-3">
                {opinion.map((v, idx) => {
                    return <div key={idx} className="rounded-xl p-1 col-span-1 border-2">
                        <p className="" key={idx}>{v}</p>
                    </div>
                })}
            </div>
            <div className="flex justify-center mt-2">
                <button
                    onClick={() => get_opinion(current_paging)}
