...
export default function ShowResultPage() {
    ...
    useEffect(() => {
        ...
        supaclient.from('all_view').select('*')
            .then(({ data, error }) => {
                if (!data) return;
                const datas: any = {};
                ages.map((v) => {
                ...
                data.map((v: any) => {
                    ...
                    datas["총괄"].questions[0][v.question_id - 1][v.answer == 1 ? 'agree' : 'disagree'] += v.count; //질문에 따라 증가
                    datas["총괄"].questions[v.gender][v.question_id - 1][v.answer == 1 ? 'agree' : 'disagree'] += v.count; //질문에 따라 증가
                    if (v.question_id == 1) {
                ...
                supaclient.from('all_types').select('*')
                    .then(({ data, error }) => {
                        if (!data) return;
                        data.map((v: any) => {
    ...
    return (<div>
        <div className="flex w-full">
            <button className={" flex-1 flex flex-col items-center transition-all duration-700 " + (page == 1 ? "bg-blue-200 text-blue-600" : "bg-gray-300  ")}
                onClick={() => setPage(1)}>
                <svg
                    ...
                    <g id="SVGRepo_iconCarrier"> <path fill="#455A64" d="M36,4H26c0,1.1-0.9,2-2,2s-2-0.9-2-2H12C9.8,4,8,5.8,8,8v32c0,2.2,1.8,4,4,4h24c2.2,0,4-1.8,4-4V8 C40,5.8,38.2,4,36,4z" /> <path fill="#ffffff" d="M36,41H12c-0.6,0-1-0.4-1-1V8c0-0.6,0.4-1,1-1h24c0.6,0,1,0.4,1,1v32C37,40.6,36.6,41,36,41z" /> <g fill="#90A4AE"> <path d="M26,4c0,1.1-0.9,2-2,2s-2-0.9-2-2h-7v4c0,1.1,0.9,2,2,2h14c1.1,0,2-0.9,2-2V4H26z" /> <path d="M24,0c-2.2,0-4,1.8-4,4s1.8,4,4,4s4-1.8,4-4S26.2,0,24,0z M24,6c-1.1,0-2-0.9-2-2s0.9-2,2-2s2,0.9,2,2 S25.1,6,24,6z" /> </g> <g fill="#CFD8DC"> <rect x="21" y="20" width="12" height="2" /> <rect x="15" y="19" width="4" height="4" /> </g> <g fill="#03A9F4"> <rect x="21" y="29" width="12" height="2" /> <rect x="15" y="28" width="4" height="4" /> </g> </g>
                </svg>
                <span className="btm-nav-label">가치 통계</span>
            </button>
            <button className={"  flex-1 flex flex-col items-center transition-all duration-700 " + (page == 2 ? "bg-teal-200 text-teal-600" : "bg-gray-300 ")}
                onClick={() => setPage(2)}>
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-5 w-5"
                ...
                <span className="btm-nav-label">기술 통계</span>
            </button>
        </div>
        <div className="flex gap-4 justify-center" >
            <div className="form-control ">
                ...
                <select className="select select-primary" defaultValue={0} onChange={select_gender_handle}>
            ...
            <div className="form-control">
                ...
                <select className="select select-primary" defaultValue={0} onChange={select_grade_handle}>
                    <option value="0">전체</option>
            ...
            <div className="form-control">
                ...
                <select className="select select-primary" value={age} onChange={select_age_handle}>
                    {grade == "초등학생" && <>
        ...
        {page == 2 && questions.length > 0 && Object.keys(current_data).includes("questions") && <div>
            {questions.map((v: any, idx: number) => {
                ...
                return (<div key={idx} className="bg-base-100 drop-shadow-2xl  rounded-2xl m-2 mb-3">
                    <div className="pt-3">
                        <span className="badge badge-primary badge-outline">문항</span>
                    ...
                    <div className="grid grid-cols-12  p-2 mt-[25px]">
                        ...
                        <div className="col-span-10 flex justify-center items-center">
                            <div className={agree_className} data-tip={agree_rate} style={{ width: `${agree_rate}%` }} />
                            <div className={disagree_className} data-tip={disagree_rate} style={{ width: `${disagree_rate}%` }} />
                        </div>
                        <div className="col-span-1 flex justify-center items-center">
                            반대
                        </div>

                        <div className="col-span-6 p-2 my-2 bg-blue-100 rounded-lg">{questions[idx].positive_text}</div>
                        <div className="col-span-6 p-2 my-2 bg-red-100 rounded-lg">{questions[idx].negative_text}</div>
                    </div>
                </div>
                )
            })}
...
const base_temp = {
    ...
    questions: [new Array(9).fill({ agree: 0, disagree: 0 }), new Array(9).fill({ agree: 0, disagree: 0 }), new Array(9).fill({ agree: 0, disagree: 0 })]
}
